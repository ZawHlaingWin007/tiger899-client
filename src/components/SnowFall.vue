<script setup>
import { onMounted, ref, nextTick } from "vue";

const snowflakes = ref([]);

onMounted(async () => {
  await nextTick();

  // Create 100 snowflakes for better coverage
  snowflakes.value = Array.from({ length: 100 }, (_, i) => {
    const size = 8 + Math.random() * 18; // 8-24px for more variety
    const animationDuration = 6 + Math.random() * 14; // 6-20 seconds (smaller = faster)
    const horizontalDrift = -30 + Math.random() * 60; // -30px to 30px horizontal movement
    // More blur for larger flakes, less for smaller ones
    const blurAmount =
      size < 12
        ? 1.2 + Math.random() * 0.8
        : size < 18
        ? 0.6 + Math.random() * 0.7
        : 0.4 + Math.random() * 0.4;
    const rotationSpeed = 180 + Math.random() * 540; // 180-720 degrees rotation

    return {
      id: i,
      left: Math.random() * 100, // Random horizontal position
      animationDuration: animationDuration,
      animationDelay: Math.random() * animationDuration, // Stagger start times
      opacity: 0.5 + Math.random() * 0.5, // 0.5-1.0 opacity for better visibility
      size: size,
      horizontalDrift: horizontalDrift,
      blur: blurAmount,
      rotationSpeed: rotationSpeed,
    };
  });
});
</script>

<template>
  <div class="fixed inset-0 pointer-events-none z-50 overflow-hidden">
    <div
      v-for="snowflake in snowflakes"
      :key="snowflake.id"
      class="snowflake"
      :style="{
        left: snowflake.left + '%',
        top: '-50px',
        animationDuration: snowflake.animationDuration + 's',
        animationDelay: snowflake.animationDelay + 's',
        opacity: snowflake.opacity,
        filter: `blur(${snowflake.blur}px)`,
        width: snowflake.size + 'px',
        height: snowflake.size + 'px',
        '--drift': snowflake.horizontalDrift + 'px',
        '--rotate': snowflake.rotationSpeed + 'deg',
      }"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        class="w-full h-full"
      >
        <path
          fill="currentColor"
          d="M13 3a1 1 0 1 0-2 0v2.586l-1.95-1.95A1 1 0 1 0 7.636 5.05L11 8.414V11H8.414L5.05 7.636A1 1 0 1 0 3.636 9.05L5.586 11H3a1 1 0 1 0 0 2h2.586l-1.95 1.95a1 1 0 1 0 1.414 1.414L8.414 13H11v2.586L7.636 18.95a1 1 0 1 0 1.414 1.414l1.95-1.95V21a1 1 0 1 0 2 0v-2.586l1.95 1.95a1 1 0 1 0 1.414-1.414L13 15.586V13h2.586l3.364 3.364a1 1 0 1 0 1.414-1.414L18.414 13H21a1 1 0 1 0 0-2h-2.586l1.95-1.95a1 1 0 1 0-1.414-1.414L15.586 11H13V8.414l3.364-3.364a1 1 0 0 0-1.414-1.414L13 5.586z"
        />
      </svg>
    </div>
  </div>
</template>

<style scoped>
.snowflake {
  position: absolute;
  color: white;
  will-change: transform;
  animation: snowfall linear infinite;
}

@keyframes snowfall {
  0% {
    transform: translateY(-100px) translateX(0) rotate(0deg);
  }
  100% {
    transform: translateY(calc(100vh + 100px)) translateX(var(--drift, 0px))
      rotate(var(--rotate, 360deg));
  }
}
</style>
